...
export default function Projects() {
  ...
  const projectSelected = projectsData.filter(
    (project) => project.type === typeSelected
  );
    ...
    <div className='w-full min-h-[400px] relative mb-36' id='projects'>
      <div className='content-project relative z-[1]'>
        ...
        <div className='relative'>
          <div className='flex items-center'>
            {typeProject.map(({ type }) => (
              <a
                href='#'
                key={type}
                className={`capitalize sm:text-base text-sm ${
                  typeSelected === type ? 'border-b-2 border-b-[#f5d2b3]' : null
                } font-medium typesRamas`}
                style={{ color: Themes.text }}
                onMouseEnter={(e) => handleMouseEnter(e)}
                onMouseLeave={handleMouseLeave}
                onClick={(e) => handleChangeSelected(e, type)}
              >
                {type}
              </a>
            ))}
          ...
          <div
            className='borderProject'
            ref={refElementProjectSelected}
            style={{ backgroundColor: Themes.cardSkill }}
          />
        </div>
        <div className='flex flex-wrap lg:justify-start justify-center'>
          {projectSelected.length > 0 ? (
            ...
            <h4
              ...
              style={{ color: Themes.text }}
            >
              No se han creado proyectos para esta sección aun. Es posible que
              el creador está estudiando las tecnologias para la creación de los
              mismos.
            </h4>
          )}
        </div>
        <div className='w-full text-center mt-10'>
          <a
            href='https://github.com/AdrianlDev1'
            className='bg-[#0284C7] py-[10px] text-white font-medium hover:bg-[#33bbff] transition-[background-color] px-10 rounded-3xl text-[15px]'
          >
            Ver mas
          </a>
        </div>
